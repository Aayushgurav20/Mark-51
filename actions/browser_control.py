...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".vanta_profiles" / browser
...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".vanta_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using VANTA profile")
                vanta = str(Path.home() / ".vanta_profiles" / "firefox_vanta")
                Path(vanta).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(vanta, **kwargs)
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".vanta_profiles" / "safari")
        ...
        # VANTA otomasyon profiline geçilir — buraya bir kez giriş yapılan
        # hesaplar sonraki oturumlarda da açık kalır.
        vanta_profile = str(Path.home() / ".vanta_profiles" / self.browser_name)
        Path(vanta_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with VANTA profile: {vanta_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(vanta_profile, **kwargs)
            self._page = await self._adopt_page()
            print(f"[Browser] ✅ Launched [{label}] with VANTA profile "
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "vanta_screenshot.png")

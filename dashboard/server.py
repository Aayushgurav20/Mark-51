"""
dashboard/server.py — VANTA Local HTTP Dashboard
...
def _make_uploads_dir() -> Path:
    ...
    for candidate in [
        Path.home() / "Downloads" / "VANTA Uploads",
        Path.home() / "Documents" / "VANTA Uploads",
        BASE_DIR / "uploads",
    ]:
        try:
...
# ── AES-256-CBC ───────────────────────────────────────────────────────────────
_AES_SALT = b'VANTA-DASHBOARD-v1'
...
def _ensure_network_access(port: int) -> None:
    ...
    if sys.platform == "win32":
        ...
        port_rule = f"VANTA Dashboard Port {port}"
        prog_rule  = "VANTA Dashboard Python"
        ...
        bat_body = "\r\n".join(bat_lines) + "\r\n"
        fd, bat_path = tempfile.mkstemp(suffix=".bat", prefix="vanta_fw_")
        try:
        ...
        try:
            ...
            if int(ret) > 32:
                ...
                print("[Dashboard] Phone connections may fail until VANTA is run as Administrator.")
...
class DashboardServer:
    ...
    def _ssl_enabled() -> bool:
        ...
        return (certs / "vanta.key").exists() and (certs / "vanta.crt").exists()
...
<p>Press <strong style="color:#dde3ed">Remote Control</strong> in VANTA to get a new QR code.</p>
</div></body></html>""")
...
<body>
<script>
  sessionStorage.setItem('vanta_token','{tok}');
  sessionStorage.setItem('vanta_key','{key}');
  localStorage.setItem('vanta_device_token','{dev_tok}');
  setTimeout(function(){{location.replace('/')}},400);
</script>
<p>Connecting to VANTA…</p>
</body></html>""")
    ...
    async def _serve_alias(self) -> None:
        ...
        ssl_key  = BASE_DIR / "config" / "certs" / "vanta.key"
        ssl_cert = BASE_DIR / "config" / "certs" / "vanta.crt"
    ...
    async def serve(self) -> None:
        ...
        print("[Dashboard] Press 'Remote Control' in VANTA UI to get the QR code.")
        await uvicorn.Server(cfg).serve()
